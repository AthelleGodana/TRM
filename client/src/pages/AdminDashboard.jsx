...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [busRes, capRes] = await Promise.all([
          api.get('/admin/buses'),
          api.get('/admin/capacity-planning')
        ]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-50">
            <h3 className="text-xl font-bold text-gray-900">Capacity Planning</h3>
            <p className="text-sm text-gray-400">Demand distribution by timeslot</p>
          </div>
          <div className="p-6 space-y-6">
            {capacityData.map(slot => (
              <div key={slot.id} className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="font-bold text-gray-700">{slot.pickupPoint.name} ({slot.pickupTime})</span>
                  <span className="font-mono text-blue-600 font-black">{slot.students.length} Students</span>
                </div>
                <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
                  <div
                    className="bg-blue-600 h-full transition-all duration-1000"
                    style={{ width: `${Math.min((slot.students.length / 40) * 100, 100)}%` }}
                  ></div>
                </div>
              </div>
            ))}
            {capacityData.length === 0 && (
              <p className="text-center text-gray-400 italic py-8">No capacity data available</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-50">
            ...
            <p className="text-sm text-gray-400">Real-time bus occupancy</p>
          </div>
                ...
                {buses.map(bus => (
                  <tr key={bus.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-gray-600">
                      {bus.students.length} / {bus.capacity}
                      <span className={`ml-2 text-xs font-black ${bus.students.length >= bus.capacity ? 'text-red-500' : 'text-green-500'}`}>
                        {bus.students.length >= bus.capacity ? '(FULL)' : '(AVAILABLE)'}
                      </span>
                    </td>
